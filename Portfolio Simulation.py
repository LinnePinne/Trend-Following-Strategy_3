...
MAX_GROSS_EXPOSURE = 1.0         # max 100% av equity i öppna positionsvärden
EQUAL_WEIGHT = True              # start: lika vikt
TARGET_GROSS_EXPOSURE = 1.0      # t.ex 1.0 = försök investera fullt när möjligt
...
def build_portfolio_mtm_cash(
    ...
    for ts in all_index:
        ...
        if ts in entries.groups:
            ...
            for _, t in block.iterrows():
                ...
                # Target notional for this entry = equity * target_gross * weight
                desired_notional = equity_now * target_gross_exposure * weights.get(mkt, 0.0)
    ...
    return equity_series, daily_returns, open_pos_series, gross_exposure_series
...
def build_strategy_daily_returns_per_market(
    ...
    for mkt, df in market_dfs.items():
        ...
        eq, daily_ret, _, _ = build_portfolio_mtm_cash(
            market_dfs={mkt: df},
...
equity_series, daily_returns, open_pos_series, gross_exposure_series = build_portfolio_mtm_cash(
    market_dfs=market_dfs,
