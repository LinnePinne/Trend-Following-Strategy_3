...
ADX_THRESHOLD = 15
ATR_K = 0.1  # testgrid: 0.0, 0.25, 0.5
...
def run_backtest_for_market(market_name: str, csv_path: str):
    ...
    for i in range(1, len(df) - 1):
        ...
        # ======================
        # ENTRY: EMA-cross + ADX + no-immediate-reversal
        # ======================
        adx_filter = adx > ADX_THRESHOLD
        cross_ema = prev_ema_fast < prev_ema_medium and ema_fast > ema_medium

        # Strukturfilter: priset måste vara "separerat" från EMA_slow
        atr = row["atr"]
        ema_sep_ok = (
                not np.isnan(atr)
                and (row["close"] - ema_medium) > (ATR_K * atr)
        )

        long_entry_signal = cross_ema and adx_filter and ema_sep_ok
